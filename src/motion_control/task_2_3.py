...
class Robot:
    def __init__(self, name):
        ...
        self.safety_status = 0 #safety state 0-5
    ...
    def build_status_message(self):
        ...
        status = {
            ...
            "safetyState": {
                "eStop": estop_state,
                "fieldViolation": self.safety_status >= 2
            },
...
def apply_safety_filter(robot: Robot, cmd):
    """
    Safety handling for Task 3 in normal mode:
    0: free, 1: warning(slow down), >=2: stop.
    """
    ...
    if safety_status == 1:
        ...
        # Reset PID memory to avoid jumps after obstacle removal.
        robot.prev_distance_error = 0.0
        robot.prev_angle_error = 0.0
        robot.sum_distance_error = 0.0
        robot.sum_angle_error = 0.0
...
def main(robot_name):
    ...
    def on_message(client, userdata, msg):
        if msg.topic == topic_pose:
            ...
            try:
                payload = json.loads(msg.payload.decode('utf-8'))
                robot.safety_status = int(payload.get("safety_status", 0))
    ...
    while True:
        ...
        cmd = apply_safety_filter(robot, cmd)
